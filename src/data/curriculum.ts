    ...
    for (let d = 0; d < 6; d++) {
      ...
      days.push({
        ...
        theory: {
          ...
          duration: "30 Mins",
          content: `In modern cyber warfare, understanding ${dayTitle} requires stepping away from simple automation scanners and getting deep inside the developer's neural pathway. When software engineers design user-centric dynamic features, they make implicit, dangerous assumptions about the inputs they handle. Under high release pressure, developers rely on default browser state or trust incoming client headers implicitly, creating highly exploitable attack surfaces.`,
          developerMindset: `The developer was trying to implement a robust, high-performance feature while strictly focusing on positive UX paths. They expected inputs to align with standardized JSON objects, safe parameters, or authenticated browser sessions. Security validation was skipped or delegated to a standard front-end framework, leaving the back-end or browser state vulnerable to deep manipulation.`,
          psychologicalError: `Security-through-convenience bias. The engineer assumed that because the custom state data or headers are not directly modifiable in standard UI controls (like text forms), users would not be able to modify them, failing to recognize that proxy tools (like Burp Suite) and headless browsers bypass all front-end interface constraints completely.`,
          attackVectors: `Direct intercept, header injection, custom protocol fuzzing, prototype pollution via global object pollution, and automated payload mapping. By modifying variables directly inside the running browser runtime or backend API requests, we bypass security boundaries entirely and leak confidential flags.`
        },
        digitalArena: {
          title: "The Digital Arena",
          duration: "90 Mins",
          labLink,
          instructions: `1. Follow the direct link to the external PortSwigger lab environment.\n2. Apply the theoretical concepts learned today to analyze the target system.\n3. Identify the vulnerability vector, intercept the traffic, inject the appropriate payload, and successfully extract the flag.\n4. Input your captured FLAG in the interactive console below to verify and complete the lab.`,
